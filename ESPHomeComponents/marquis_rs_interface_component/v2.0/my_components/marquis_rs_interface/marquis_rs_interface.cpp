...
void MarquisRSInterface::loop() {
  // Process only one frame per loop iteration to prevent blocking
  if (frame_read_index_ != frame_write_index_) {
    uint32_t frame = frame_buffer_[frame_read_index_];
    frame_read_index_ = (frame_read_index_ + 1) % FRAME_BUFFER_SIZE;
    ...
    // Enhanced flashing detection: track display changes within 2-second windows
    bool display_changed = (display != last_display_);
    if (display_changed) {
      last_display_change_time_ = now;
    ...
    // Rate limiting: publish at most every 100ms to prevent overwhelming Home Assistant
    bool should_publish = false;
    if (frame == last_valid_frame_) {
      if (++repeat_count_ >= 3) {
        should_publish = (now - last_publish_time_ >= 100);
        repeat_count_ = 0;
      }
    } else {
      repeat_count_ = 1;  // Start counting this new frame
...
void IRAM_ATTR MarquisRSInterface::isr_read_bit() {
  ...
  // Use digitalRead for Arduino framework compatibility (delay already removed for timing)
  ...
  if (bit_index_ >= 21) {
    bit_index_ = 0;
    // Write to ring buffer (thread-safe: ISR writes, main loop reads)
    uint8_t next_write = (frame_write_index_ + 1) % FRAME_BUFFER_SIZE;
    if (next_write != frame_read_index_) {  // Prevent overflow
      frame_buffer_[frame_write_index_] = current_frame_;
      frame_write_index_ = next_write;
    }
  }
}
